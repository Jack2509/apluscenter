...
$ten_input=addslashes($_POST['ten']);
				...
				<table border="1" bordercolor="#0099CC" align="center" cellpadding="5px" cellspacing="5px" width="100%">';
					if(is_array($_SESSION['cart']))
					{
						...
						for($i=0;$i<$max;$i++){
							...
							if($q==0) continue;
							$body.='<tr><td>'.($i+1).'</td>';
							$body.='<td> <a href="http://'.$config_url.'/san-pham/'.changeTitle($pname).'.html" target="_blank">';
							if ($color>0) {$body.='<img src="http://'.$config_url.'/upload/product/'.get_color($color).'" width="70" />';
						} else {
							$body.='<img src="http://'.$config_url.'/upload/product/'.get_thumb($pid).'" width="70" />';
						}
						$body.='</a></td>';
...
<tr><td colspan="2" align="left">
	...
	<td> <tr>'; 
		...
		// $mathanhvien=$_SESSION['login_web']["id"];
		// dump($mathanhvien);
		$macolor=$color;
		// dump($macolor);
