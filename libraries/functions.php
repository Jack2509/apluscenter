...
function get_gia($gia,$giamgia,$donvitien,$lienhe)
{
	if($gia>0){
		if($giamgia>0){
			$gia = $gia - $gia*$giamgia/100;
			$result = number_format($gia,0,",",".").$donvitien;
		}else{
			$result = number_format($gia,0,",",".").$donvitien;
		}
	}else{
...
function getCurrentPageURL_index() {
    $pageURL = 'http';
