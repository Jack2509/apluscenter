...
<form name="supplier" id="validate" class="form" action="index.php?com=order&act=save" method="post" enctype="multipart/form-data">
    ...
    <tbody>
     <?php      
				...
				$color=$result_ctdonhang[$i]['id_mau'];
					
					
				$pname=get_product_name($pid);
				$pphoto=get_color($color);
				$pphoto1=get_thumb($pid);
        ...
        <tr id="productct<?=$result_ctdonhang[$i]['id']?>">
          ...
          <td><?=$pname?></td>
           <td>
           <?php if ($color>0) {?>
           <img src="<?=_upload_product.$pphoto?>" height="100"  />
           <?php } else { ?>
           <img src="<?=_upload_product.$pphoto1?>" height="100"  />
           <?php } ?>
