<script type="text/javascript">
  ...
  function select_list()
  {
    ...
    window.location ="index.php?com=hoidap&act=man&type=<?=$_GET['type']?>&id_list="+a.value; 
    return true;
  }
...
<?php
  ...
  {
    $sql="select * from table_hoidap_list where type='".$_GET['type']."' order by stt asc";
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    <thead>
      <tr>
        ...
        <?php } ?>
        <td class="sortCol"><div>Câu hỏi<span></span></div></td>
       ...
       <td>
        ...
        <?php if($config_list=='true'){ ?>
         <td align="center">
          <?php
            $d->reset();
            $sql = "select ten_vi from table_hoidap_list where id='".$items[$i]['id_list']."'";
        ...
        <td class="title_name_data">
            <a href="index.php?com=hoidap&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><?=$items[$i]['tieude']?></a>
        </td>
        ...
        <td class="actBtns">
            <a href="index.php?com=hoidap&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" title="" class="smallButton tipS" original-title="Sửa bài viết"><img src="./images/icons/dark/pencil.png" alt=""></a>

            <a href="index.php?com=hoidap&act=delete&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa bài viết"><img src="./images/icons/dark/close.png" alt=""></a>
        </td>
      </tr>
         <?php } ?>
                </tbody>
