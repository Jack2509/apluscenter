...
#====================================
	case "man_loai":
	get_loais();
	$template = "baiviet/loai/items";
	break;
	case "add_loai":		
	$template = "baiviet/loai/item_add";
	break;
	case "edit_loai":		
	get_loai();
	$template = "baiviet/loai/item_add";
...
function get_loais(){
	...
	if($_REQUEST['hienthi']!='')
	{
		...
		$id_up = $_REQUEST['hienthi'];
		$sql_sp = "SELECT id,hienthi FROM table_baiviet_loai where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$hienthi=$cats[0]['hienthi'];
		if($hienthi==0)
		{
		$sqlUPDATE_ORDER = "UPDATE table_baiviet_loai SET hienthi =1 WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
		}
		else
		{
		$sqlUPDATE_ORDER = "UPDATE table_baiviet_loai SET hienthi =0  WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
    ...
    $url = "index.php?com=baiviet&act=man_loai&type=".$_GET['type']."".$link_add;
	$paging = pagination($where,$per_page,$page,$url);
}
...
function get_loai(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_loai&type=".$_GET['type']);
	
	$sql = "select * from #_baiviet_loai where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=baiviet&act=man_loai&type=".$_GET['type']);
	$item = $d->fetch_array();
}
...
function save_loai(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_loai&type=".$_GET['type']);
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	if($id){

		if($photo = upload_image("file", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_baiviet,$file_name)){
			$data['photo'] = $photo;
			$data['thumb'] = create_thumb($data['photo'], 145, 100, _upload_baiviet,$file_name,1);	
			$d->setTable('baiviet_loai');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=baiviet&act=man_loai&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=baiviet&act=man_loai&type=".$_GET['type']);
	}else{
		...
		$d->setTable('baiviet_loai');
		if($d->insert($data))
			redirect("index.php?com=baiviet&act=man_loai&type=".$_GET['type']);
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=baiviet&act=man_loai&type=".$_GET['type']);
	}
}
...
function delete_loai(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id,photo,thumb from #_baiviet_loai where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_baiviet.$row['photo']);
			...
			$sql = "delete from #_baiviet_loai where id='".$id."'";
			$d->query($sql);
		}
		if($d->query($sql))
			redirect("index.php?com=baiviet&act=man_loai&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=baiviet&act=man_loai&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
	} elseif (isset($_GET['listid'])==true){
		$listid = explode(",",$_GET['listid']); 
		for ($i=0 ; $i<count($listid) ; $i++){
			$idTin=$listid[$i]; 
		...
		transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_loai&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
	}
}
...
function save_list(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_baiviet,$file_name)){
			$data['photo'] = $photo;
			$data['thumb'] = create_thumb($data['photo'], _width_thumb_l, _height_thumb_l, _upload_baiviet,$file_name,_style_thumb);	
